...
void MedianFilter(int start, int end, int* image, int width, int height, int window_size,int index)
{
...
int main()
{
	...
	while (NumberOfPictures--) {
		...
		MedianFilter(start, end, imageData, ImageWidth, ImageHeight, window_size, 0);
		...
		if (Rank == 0) {
			drawing(image, ImageWidth, ImageHeight, NumberOfPictures);
		}
		MPI_Bcast(&NumberOfPictures, 1, MPI_INT, 0, MPI_COMM_WORLD);
	}
	if (Rank == 0) {
